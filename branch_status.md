...
| `bugfix/song-swap-timeout`                      | active     | testable    | Song swap busy-wait hangs on infinite clips + E369 ModelStack race during swap — caps quantization to 8 bars, 60s safety timeout, audio lock during swap | none (demo mode + MIDI stress test reproduces) | #114 |
...
| ----------------------------------------- | ------ | ----------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | -------------- |
| `feature/enable-l2-cache`             | active | has-test    | Enable PL310 L2 data cache (~60% SDRAM speedup) + DMA cache maintenance + FatFS buffer alignment | `tests/unit/cache_maintenance_tests.cpp` (x86), on-device DMA coherency tests in self-test framework | -- |
| `feature/l2-cache-benchmark`          | active | has-test    | L2 cache A/B benchmark in SETTINGS menu (SDRAM, SD read, SD write+verify) | on-device benchmark action (SETTINGS > L2 Cache Bench) | #133 |
